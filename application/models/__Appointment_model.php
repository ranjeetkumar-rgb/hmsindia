...
class Appointment_model extends CI_Model
{
    ...
    function my_appointments_count($center, $start_date, $end_date, $patient_id, $patient_name, $status, $doctor){
		$conditions = "";
       /* if(!empty($center)){
    ...
    function my_appointments_pagination($limit, $page, $center, $start_date, $end_date, $patient_id, $patient_name, $status, $doctor){
		if(empty($page)){
			$offset = 0;
		}else{
			$offset = ($page - 1) * $limit;
	...
	function patient_procedure($appointment_id){
		$result = array();
		$sql = "Select patient_id, receipt_number, appointment_id, ID as patient_procedure_id, data from ".$this->config->item('db_prefix')."patient_procedure where appointment_id='$appointment_id' order by ID desc limit 1";
		$q = $this->db->query($sql);
        $result = $q->result_array();
        if (!empty($result))
        {
