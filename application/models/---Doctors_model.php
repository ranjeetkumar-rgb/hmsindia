...

{
	...
	function consultation_medicine(){
		...
		$sql_condition = '';
	  	...
	  	$sql ="Select * from ".$this->config->item('db_prefix')."stocks where status='1' and expiry >= DATE(now()) ORDER by expiry ASC and follow_up_appointment='1'";
...
function patient_pcpndt_count($center, $start_date, $end_date, $iic_id, $test_type,$ID){
	...
	function patient_pcpndt_list_patination($limit, $page, $center, $start_date, $end_date, $iic_id, $test_type, $ID){
		$investigation_result = array();
		$conditions = '';
		if(empty($page)){
			$offset = 0;
