...
class Order_Model extends CI_Model{
	...
	function get_my_purchase_internal_orders_data($purchase_order, $start_date, $end_date, $item_name){
		$orders_result = array();
		$conditions = '';
		if (!empty($purchase_order)){
			$conditions .= " and purchase_order='$purchase_order'";
        ...
        $consultation_sql = "Select * from ".$this->config->item('db_prefix')."internal_orders where 1 ".$conditions."";
		$q = $this->db->query($consultation_sql);
		return $q->num_rows();
		
	}
	
	function get_my_internal_orders_count($limit, $page, $purchase_order, $start_date, $end_date, $item_name){
		$orders_result = array();
		$conditions = '';
		if(empty($page)){
			$offset = 0;
	    ...
	    $consultation_sql = "Select * from ".$this->config->item('db_prefix')."internal_orders where 1".$conditions." order by ID desc limit ". $limit." OFFSET ".$offset."";
		$consultation_q = $this->db->query($consultation_sql);
		$orders_result = $consultation_q->result_array();
		return $orders_result;
	}
	...
	function export_stocks_reports($employee_number, $center_number, $patient_id, $start_date, $end_date, $item_name, $batch_number, $type){
		...
		if (!empty($start_date) && !empty($end_date)){
			...
			$conditions .= " and date_of_purchase between '".$start_date."' AND '".$end_date."' ";
		}
		else if (!empty($start_date) && empty($end_date)){
			$conditions .= " and date_of_purchase='$start_date'";
		}
		else if (empty($start_date) && !empty($end_date)){
			$conditions .= " and date_of_purchase='$end_date'";
		}
		if(!empty($item_name)){
		...
		$investigation_sql = "Select * from ".$this->config->item('db_prefix')."vendor_billing where 1 $conditions order by date_of_purchase desc";
		...
		if(!empty($investigation_result)){
			...
			$response[] = array(
				'purchase_po_no' => $val['purchase_po_no'],
				'po_date' => $val['po_date'],
				'order_number' => $val['order_number'],
				'vendor_billing' => $val['vendor_billing'],
				...
				'mrp' => $val['mrp'],
				'rate_per_unit' => $val['rate_per_unit'],
				'total_purchase_value_excl_gst' => $val['total_purchase_value_excl_gst'],
				'freight_forwarding_charges' => $val['freight_forwarding_charges'],
				'discount_rate' => $val['discount_rate'],
				'discount_amt' => $val['discount_amt'],
				'free_quantity' => $val['free_quantity'],
				'total_purchase_after_discount_exculding_gst' => $val['total_purchase_after_discount_exculding_gst'],
				'total_purchase_value_incl_gst' => $val['total_purchase_value_incl_gst'],
				'centre_location' => $val['centre_location'],
...
function get_purchase_orders_data($start_date, $end_date, $item_name, $batch_number, $invoice_no,$purchase_po_no,$vendor_name,$brand_name){
		$orders_result = array();
		$conditions = '';
		if (!empty($start_date) && !empty($end_date)){
			$conditions .= " and date_of_receiving between '".$start_date."' AND '".$end_date."' ";
		}
		else if (!empty($start_date) && empty($end_date)){
			$conditions .= " and date_of_receiving='$start_date'";
		}
		else if (empty($start_date) && !empty($end_date)){
			$conditions .= " and date_of_receiving='$end_date'";
		}
		if (!empty($item_name)){
			$conditions .= " and item_name like '%$item_name%'"; 
		}
		...
		if (!empty($purchase_po_no)){
			$conditions .= " and purchase_po_no='$purchase_po_no'";
		}
		if (!empty($vendor_name)){
			$conditions .= " and vendor_name='$vendor_name'";
		}
		if (!empty($brand_name)){
			$conditions .= " and brand_name='$brand_name'";
		}
...
function get_purchase_orders_count($limit, $page, $start_date, $end_date, $item_name, $batch_number, $invoice_no, $purchase_po_no,$vendor_name,$brand_name){
		$orders_result = array();
		$conditions = '';
		if(empty($page)){
			$offset = 0;
	...
	function get_my_internal_orders_data($po_number, $item_name){
		$data = array();
		$conditions = '';
		if (!empty($po_number)){
			$conditions .= " and po_number='$po_number'";
	...
	function get_my_orders_internal_data_count($limit, $page, $po_number, $item_name){
		$data = array();
		$conditions = '';
		if(empty($page)){
			$offset = 0;
	...
	function approve_purchase_order($order_number){
		$sql = "UPDATE `".$this->config->item('db_prefix')."orders` SET `purchase_order`='1', `update_date`='".date("Y-m-d H:i:s")."' WHERE `order_number`='".$order_number."'";
        $this->db->query($sql);
        return 1;
	}
	...
	function approve_internal_purchase_order($order_number){
		$sql = "UPDATE `".$this->config->item('db_prefix')."internal_orders` SET `purchase_order`='1', `update_date`='".date("Y-m-d H:i:s")."' WHERE `order_number`='".$order_number."'";
        $this->db->query($sql);
        return 1;
	}
	
	function disapprove_internal_purchase_order($order_number){
		$sql = "UPDATE `".$this->config->item('db_prefix')."internal_orders` SET `purchase_order`='2' WHERE `order_number`='".$order_number."'";
        $this->db->query($sql);
        return 1;
	}
	
	function order_internal_place_date($order_number){
		$sql = "UPDATE `".$this->config->item('db_prefix')."internal_orders` SET `order_place`='".date("Y-m-d")."' WHERE `order_number`='".$order_number."'";
        $this->db->query($sql);
        return 1;
	}
    ...
    {	
		...
		$quantity = $data['quantity'];unset($data['quantity']);
	    $sql = "UPDATE `".$this->config->item('db_prefix')."stocks` SET `quantity` = `quantity`+".$quantity.",`item_name`='".$data['item_name']."',`company`='".$data['company']."',`vendor_number`='".$data['vendor_number']."',`brand_name`='".$data['brand_name']."',`price`='".$data['price']."',`vendor_price`='".$data['vendor_price']."',`expiry`='".$data['expiry']."',`expiry_day`='".$data['expiry_day']."',`batch_number`='".$data['batch_number']."',`date_of_purchase`='".$data['date_of_purchase']."',`invoice_no`='".$data['invoice_no']."' WHERE `item_number`='".$item."'";
        $this->db->query($sql);
        return 1;
    }
	...
	function insert_internal_replaced_order_item($centeral_order, $data, $item_id, $center_order){
	 $sql = "INSERT INTO `" . $this->config->item('db_prefix') . "center_orders` SET ";
		$sqlArr = array();
		foreach( $data as $key=> $value )
		{
		...
		{
			$sql = "";
		 $sql = "INSERT INTO `" . $this->config->item('db_prefix') . "internal_orders` SET ";
			$sqlArr = array();
			foreach( $centeral_order as $key=> $value )
			{
				$sqlArr[] = " $key = '".addslashes($value)."'"	;
	...
	public function update_internal_admin_order_status($order_id, $action_type)
    {
		$replaced = "";
	if($action_type == "insert"){ $replaced = "`status`='1', received='1', replaced='1'"; }else {$replaced = "`status`='1', received='1'";}
	 $sql = "UPDATE `".$this->config->item('db_prefix')."internal_orders` SET ".$replaced." WHERE `ID`='".$order_id."'";
        $this->db->query($sql);
        return 1;
    }
...
//		var_dump($result);die;
	...
	function get_purchase_internal_item_data($item){
		$result = array();
		$sql_condition = '';
		$sql = "Select * from ".$this->config->item('db_prefix')."internal_orders where ID='".$item."'";
        $q = $this->db->query($sql);
        $result = $q->result_array();
        if (!empty($result))
        {
	...
	function get_product_id($product){
		$result = array();
		$sql_condition = '';
		$sql = "Select product_id from ".$this->config->item('db_prefix')."stocks where item_number='".$product."'";
        $q = $this->db->query($sql);
        $result = $q->result_array();
        if (!empty($result))
        {
	...
	function get_generic_name($product){
		$result = array();
		$sql_condition = '';
		$sql = "Select generic_name from ".$this->config->item('db_prefix')."stocks where item_number='".$product."'";
        $q = $this->db->query($sql);
        $result = $q->result_array();
        if (!empty($result))
        {
            return $result[0]['generic_name'];
        }
        else
        {
            return $product;
	...
	function get_gstdivision($product){
		$result = array();
		$sql_condition = '';
		$sql = "Select gstdivision from ".$this->config->item('db_prefix')."stocks where item_number='".$product."'";
        $q = $this->db->query($sql);
        $result = $q->result_array();
        if (!empty($result))
        {
            return $result[0]['gstdivision'];
        }
        else
        {
            return $product;
	...
	function update_purchase_item($po_number,$order_number, $order_quantity, $company, $vendor_number, $brand_name, $mrp, $hsn, $pack_size, $gstrate ,$vendor_price, $ship_to, $bill_to,$purchase_order, $order_qty_pack, $total_vendor_price){
	    $sql = "UPDATE ".$this->config->item('db_prefix')."orders SET `order_quantity` = '$order_quantity', `company` = '$company', `vendor_number` = '$vendor_number', `brand_name` = '$brand_name', `mrp` = '$mrp', `hsn` = '$hsn', `pack_size` = '$pack_size',`gstrate` = '$gstrate',`vendor_price` = '$vendor_price',`ship_to` = '$ship_to',`bill_to` = '$bill_to', purchase_order ='$purchase_order', order_qty_pack='$order_qty_pack', total_vendor_price='$total_vendor_price', status ='0', `update_date`='".date("Y-m-d H:i:s")."' where  po_number='".$po_number."'";
        $this->db->query($sql);
		return 1;
	}
	
	function get_orders_by_po($po_number){
		$result = array();
		$sql = "SELECT * FROM ".$this->config->item('db_prefix')."orders WHERE po_number = '$po_number' ORDER BY ID ASC";
		$q = $this->db->query($sql);
		$result = $q->result_array();
		if(!empty($result)){
			return $result;
