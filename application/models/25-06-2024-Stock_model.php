...
class Stock_model extends CI_Model
{
	...
	public function add_central_stock_report($data, $item_number, $item_name, $batch_number, $curruntyquantity, $expiry, $expiry_day, $add_date, $employee_number, $vendor_price, $mrp, $item_qty, $gstrate, $gstdivision,  $closingstock, $center_number)
    {   
		$sql = "INSERT INTO `".$this->config->item('db_prefix')."central_stock_report` (`item_number`,`item_name`,`batch_number`,`openstock`,`expiry`,`expiry_day`,`add_date`,`employee_number`,`vendor_price`, `quantity_in`, `mrp`, `gstrate`, `gstdivision`, `enddate`, `quantity_out`, `closingstock`, `type`,`center_number`) VALUES ('$item_number','$item_name','$batch_number','$curruntyquantity','$expiry','$expiry_day','".date("Y-m-d H:i:s")."','$employee_number','$vendor_price','$item_qty','$mrp','$gstrate','$gstdivision','".date("Y-m-d H:i:s")."','$item_qty','$closingstock','Center In','$center_number')";
	...
	public function add_central_report($data, $product_id, $item_name, $batch_number, $expiry, $expiry_day, $add_date, $vendor_price, $mrp, $gstrate, $gstdivision, $quantity)
    
	{ 
		$sql = "INSERT INTO `".$this->config->item('db_prefix')."central_stock_report` (`item_number`,`item_name`,`batch_number`,`expiry`,`expiry_day`,`add_date`,`vendor_price`,`mrp`,`gstrate`,`gstdivision`,`closingstock`,`enddate`,`type`) VALUES ('$product_id','$item_name','$batch_number','$expiry','$expiry_day','".date("Y-m-d H:i:s")."','$vendor_price','$mrp','$gstrate','$gstdivision','$quantity','".date("Y-m-d H:i:s")."','Central Stocks New Entry')";
	...
	public function update_item($data, $product_id, $invoice_no, $no_of_item, $brand_name, $vendor_number, $batch_number, $vendor_price, $hsn, $pack_size, $gstrate, $gstdivision, $expiry)
    {
		//var_dump($data);die;
		$sql = "UPDATE hms_stocks SET `product_id` = '$product_id', `invoice_no` = '$invoice_no', `no_of_item` = '$no_of_item', `brand_name` = '$brand_name', `vendor_number` = '$vendor_number', `quantity` = quantity+".$data['quantity'].", `safety_stock` = '".$data['safety_stock']."', `price` = '".$data['price']."', `batch_number` = '".$batch_number."', `expiry` = '$expiry', `expiry_day` = '".$data['expiry_day']."', `order_qty` = '".$data['order_qty']."', `category` = '".$data['category']."', `vendor_price` = '".$data['vendor_price']."',`pack_size` = '".$data['pack_size']."',`hsn` = '".$data['hsn']."',`gstrate` = '".$data['gstrate']."',`gstdivision` = '".$data['gstdivision']."', `status` = '1'";
	...
	function get_center_injection_list(){
		...
		$sql_condition = '';
		if($_SESSION['logged_stock_manager']['center']){
		$sql = "Select * from ".$this->config->item('db_prefix')."center_stocks where category='1565461619' AND center_number='".$_SESSION['logged_stock_manager']['center']."' AND employee_number='".$_SESSION['logged_stock_manager']['employee_number']."' AND status='1' AND quantity > 0 order by expiry asc";
        }else{
		$sql = "Select * from ".$this->config->item('db_prefix')."center_stocks where category='1565461619' AND center_number='".$_SESSION['logged_billing_manager']['center']."' AND employee_number='".$_SESSION['logged_billing_manager']['employee_number']."' AND status='1' AND quantity > 0 order by expiry asc";
        }
	...
	function billing_purchase_item_insert($data){
		echo $sql = "INSERT INTO `" . $this->config->item('db_prefix') . "purchase_order` SET ";
		$sqlArr = array();
		foreach( $data as $key=> $value )
		{
			$sqlArr[] = " $key = '".addslashes($value)."'";
...
/*****All Center Stock*****/
	...
	function export_all_center_stocks($employee_number, $start, $end, $generic_name, $item_name){
	    ...
	    $investigation_sql = "Select DISTINCT item_number, company,employee_number, item_name,batch_number,hsn,brand_name,vendor_number,generic_name,quantity,safety_stock,order_qty,category,status from ".$this->config->item('db_prefix')."center_stocks where 1 $conditions order by rand() desc";
        $investigation_q = $this->db->query($investigation_sql);
        $investigation_result = $investigation_q->result_array();
        if(!empty($investigation_result)){
            foreach($investigation_result as $key => $val){
 ...
 function get_central_stocks_patination($limit, $page, $start_date, $end_date, $generic_name, $item_name){
    ...
    function get_stocks_reports_patination($limit, $page, $employee_number, $center_number, $start_date, $end_date, $item_name, $batch_number){
		$conditions = '';
		if(empty($page)){
			$offset = 0;
		}else{
		...
		if (!empty($batch_number)){
		 ...
		 $investigation_sql = "Select * from ".$this->config->item('db_prefix')."central_stock_report where 1".$conditions." order by expiry_day desc, enddate desc limit ". $limit." OFFSET ".$offset."";
	...
	function get_stocks_reports($employee_number, $center_number, $start_date, $end_date, $item_name, $batch_number){
		$conditions = '';
		if(!empty($employee_number)){
			$emp_number = implode(', ', $employee_number);
			$conditions .= ' and employee_number  in ('.$emp_number.')';
	...
	function consumption_list_patination($limit, $page, $start_date, $end_date, $patient_id,$type){
		$conditions = '';
		if(empty($page)){
			$offset = 0;
		}else{
	...
	function consumption_list_count($start_date, $end_date, $patient_id,$type){
		$conditions = '';
		if (!empty($type)){
			$conditions .= " and type like '%$type%'";
		}
