 ...
 function get_central_stocks_patination($limit, $page, $start_date, $end_date, $generic_name, $item_name){
	...
	function get_stocks_reports($employee_number, $center_number, $start_date, $end_date, $item_name, $batch_number){
 	...
 	function export_stocks_reports($employee_number, $add_date, $enddate, $item_name, $batch_number){
	    ...
	    $investigation_sql = "Select * from ".$this->config->item('db_prefix')."central_stock_report where 1 $conditions order by rand() desc";
        $investigation_q = $this->db->query($investigation_sql);
        $investigation_result = $investigation_q->result_array();
        if(!empty($investigation_result)){
            foreach($investigation_result as $key => $val){		
	...
	function export_consumption_medicine($start, $end, $employee_number, $type){
		...
		$investigation_sql = "Select DISTINCT patient_id, on_date, data, receipt_number from ".$this->config->item('db_prefix')."patient_procedure where 1 $conditions";
        $investigation_q = $this->db->query($investigation_sql);
        ...
        if(!empty($consuption_result)){
            foreach($consuption_result as $key => $val){
				...
				// print_r($sub_procedure = implode(',', $medicine_name_arr ));
	...
	function export_consumption_injections($start, $end, $employee_number, $type){
		$consuption_result = $response = array();
        $conditions = '';
		if(!empty($employee_number)){
			$conditions .= ' and employee_number="'.$employee_number.'"';
        ...
        $consuption_result = $injections_q->result_array();
        if(!empty($consuption_result)){
            foreach($consuption_result as $key => $val){
				$_data = unserialize($val['data']);
				$injections_name_arr = [];
				...
				foreach($_data as $value1){
					foreach($value1 as $value2){
						foreach($value2 as $value3){
							...
							array_push($injections_stock_arr,$value3['injections_price']);
						}
					}
				}
				...
				foreach($_injections_name_arr as $_key => $_value){
					...
					$response[] = array(
                        ...
                        'patient_id' => $val['patient_id'],
				        'receipt_number' => $val['receipt_number'],
                        'final__injections'=> $final_injections123,
