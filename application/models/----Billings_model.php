...
class Billings_model extends CI_Model
{
	function get_billings_list(){
		...
		$consultation_sql = "Select * from ".$this->config->item('db_prefix')."consultation";
        $consultation_q = $this->db->query($consultation_sql);
        $consultation_result = $consultation_q->result_array();
		
		$investigate_sql = "Select * from ".$this->config->item('db_prefix')."patient_investigations";
        $investigate_q = $this->db->query($investigate_sql);
        $investigate_result = $investigate_q->result_array();
		
		$procedure_sql = "Select * from ".$this->config->item('db_prefix')."patient_procedure";
	...
	function patient_payments($data){
		...
		$procedure_sql = "Select receipt_number, payment_done, fees, remaining_amount, billing_from, billing_at from ".$this->config->item('db_prefix')."patient_procedure where patient_id='".$patient_id."' AND status='approved'";
        $procedure_q = $this->db->query($procedure_sql);
        $procedure_result = $procedure_q->result_array();
				
		$consultation_sql = "Select receipt_number, payment_done, fees, remaining_amount, billing_from, billing_at from ".$this->config->item('db_prefix')."consultation where patient_id='".$patient_id."' AND status='approved'";
        $consultation_q = $this->db->query($consultation_sql);
        $consultation_result = $consultation_q->result_array();
				
		$investigation_sql = "Select receipt_number, payment_done, fees, remaining_amount, billing_from, billing_at from ".$this->config->item('db_prefix')."patient_investigations where patient_id='".$patient_id."' AND status='approved'";
		...
		if(!empty($patient_result)){
			$response = array('remaining_billing' => $remaining_billing, 'patient_id' =>$patient_id, 'wife_name' =>$patient_result[0]['wife_name'], 'patient_phone' =>$patient_result[0]['patient_phone'], 'wife_email' =>$patient_result[0]['wife_email'], 'nationality' => $patient_result[0]['nationality'], 'status' => 1);
		}else{
	...
	function ajax_center_billing_date_data($start, $end, $center){
		...
		$consultation_sql = "Select * from ".$this->config->item('db_prefix')."consultation where on_date between '".$start."' AND '".$end."' AND billing_at='".$center."'";
        $consultation_q = $this->db->query($consultation_sql);
        $consultation_result = $consultation_q->result_array();
		if(!empty($consultation_result)){
			$consultation_result = $consultation_result;
		...
		$investigate_sql = "Select * from ".$this->config->item('db_prefix')."patient_investigations where on_date between '".$start."' AND '".$end."' AND billing_at='".$center."'";
        $investigate_q = $this->db->query($investigate_sql);
        $investigate_result = $investigate_q->result_array();
		if(!empty($investigate_result)){
			$investigate_result = $investigate_result;
		...
		$procedure_sql = "Select * from ".$this->config->item('db_prefix')."patient_procedure where on_date between '".$start."' AND '".$end."' AND billing_at='".$center."'";
        $procedure_q = $this->db->query($procedure_sql);
        $procedure_result = $procedure_q->result_array();
		if(!empty($procedure_result)){
			$procedure_result = $procedure_result;
	...
	function get_medicine_name($medicine){		
		$result = array();
		$sql = "Select item_name, company, brand_name from ".$this->config->item('db_prefix')."consultation_medicine where item_number='".$medicine."'";
        $q = $this->db->query($sql);
        $result = $q->result_array();
        if (!empty($result))
        {
