<?php
	...
	$sql = "Select * from ".$this->config->item('db_prefix')."donor where donor_patient_id='".$patient_id."' and type='Donor'";
	$select_result = run_select_query($sql);
	    
	$sql2 = "Select * from ".$this->config->item('db_prefix')."procedures where code='IP01D'";
  ...
  <div class="row">
    ...
    <div class="table-responsive">
        <table class="table table-striped table-bordered table-hover dataList" id="">
		  ...
		  <?php if (!empty($select_result['patient_id'])) { ?> 
		   <tbody>
		
				<tr>
					<td style="border: 1px solid black; border-collapse: collapse;padding:5px; text-align:left;" class="role"><?php echo get_procedure_data($select_result2['ID']); ?></td>
					<td style="border: 1px solid black; border-collapse: collapse;padding:5px; text-align:left;"><?php echo $select_result2['code']?></td>
					<td style="border: 1px solid black; border-collapse: collapse;padding:5px; text-align:left;">
						<a procedure_id="<?php echo $select_result2['ID']?>" href="javascript:void(0);" appointment_id="<?php echo $appointment_id; ?>" class="upload_record_donor btn btn-large">Upload record</a>
						<div class="procedure_form_div" id="procedure_form_<?php echo $select_result2['ID']?>"></div>
					</td>
					<td style="border: 1px solid black; border-collapse: collapse;padding:5px; text-align:left;">
						<a href="javascript:void(0);" procedure_id="<?php echo $val['sub_procedure']; ?>" class="check_record btn btn-large">Check record</a>
						<div style="display:none;" class="check_form_div" id="check_form_<?php echo $val['sub_procedure']; ?>">
						<?php $form_list = get_prodecure_forms($val['sub_procedure']); //var_dump($form_list);die; 
							  ...
							  foreach($form_list as $ks => $vls){
								...
								if(isset($_SESSION['logged_doctor'])){
									if($form_details['form_for'] != "lab_procedure"){
										$check_form_data = check_form_data($patient_id, $form_details['form_area']); //var_dump($form_details['form_area']);die;
										if(count($check_form_data) > 0){
											$html .= "<a target='_blank' href='".base_url('check_procedure_form/'.$vls['form_id'].'/'.$appointment_id.'/'.$val['sub_procedure'])."'>".$form_details['form_name']."</a> | ";
										}
									}
								}
							  }
...
$(document).on('click',".upload_record_donor",function(e) {
	$('.check_form_div').hide();
	var procedure_id = $(this).attr('procedure_id');
	var appointment_id = $(this).attr('appointment_id');
	$('.procedure_form_div').hide();
	...
	$.ajax({
		url: '<?php echo base_url('doctors/procedure_upload_donor')?>',
		data: {'procedure_id':procedure_id, 'appointment_id':appointment_id, 'patient_id':'<?php echo $patient_id; ?>'},
		dataType: 'json',
		method:'post',
		success: function(data)
		{
