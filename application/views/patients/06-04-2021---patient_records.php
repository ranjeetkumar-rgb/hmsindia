...
<div class="col-sm-12 col-xs-12  panel panel-piluku ml-5">
  ...
  <hr/>
  <?php $procedure_billing = check_procedure_billing($appointment_id); //var_dump($procedure_billing);die;
  ...
  <div class="row">
    ...
    <div class="table-responsive">
        <table class="table table-striped table-bordered table-hover dataList" id="">
          ...
          <tbody>
			<?php 
				  foreach($procedure_datas['patient_procedures'] as $ky => $val){ $procedure_assign = have_procedure_assign($val['sub_procedure'], 'daycare_procedure');
					if($procedure_assign){  ?>
				<tr>
					...
					<td style="border: 1px solid black; border-collapse: collapse;padding:5px; text-align:left;">
						...
						<?php $form_list = get_prodecure_forms($val['sub_procedure']); //var_dump($form_list);die; 
							  $html = "";
							  foreach($form_list as $ks => $vls){ $form_details = get_prodecure_form($vls['form_id']); 
								if(isset($_SESSION['logged_doctor'])){
									if($form_details['form_for'] != "lab_procedure"){
										...
										if(count($check_form_data) > 0){
											$html .= "<a target='_blank' href='".base_url('check_procedure_form/'.$vls['form_id'].'/'.$val['sub_procedure'].'/'.$appointment_id.'/'.$patient_prodedures['ID'])."'>".$form_details['form_name']."</a> | ";
										}
									}
								}
							  }
