...
if ($result && isset($result['investigations'])) {
    ...
    if ($pro_result && isset($pro_result['data'])) {
		...
		</tr><?php  $grand_total +=  $subtotal; $male_med_count++; }} ?>
...
foreach ($applicablemedicine as $med) {
    ...
    if (isset($medicineData[$med])) {
        ...
        <tr>
            ...
            <td><p><?= $med ?></p></td>
            <td><p>Pack Size: <?= $packSize ?></p></td>
            <td><p>MRP: ₹<?= number_format($mrp, 2) ?></p></td>
            <td><p><?= $days ?> days</p></td>
            <td><p><strong>Total: ₹<?= number_format($totalPrice, 2) ?></strong></p></td>
        </tr>
        <?php
    }
}
