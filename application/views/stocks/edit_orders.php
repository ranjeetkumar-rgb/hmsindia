            ...
            <div id="main_div">
            	<div class="row">            
                   <div class="form-group col-sm-6 col-xs-12">
                        ...
                        <input value="<?php echo $po_number; ?>" placeholder="Receipt number" id="po_number" name="po_number" readonly="readonly" type="text" class="form-control required_value" required>                        
				   </div>
                   <div class="col-sm-6 col-xs-12" style="margin-top:10px;">
					...
					<select name="vendor_number" class="form-control" required id="vendor_number">
						...
						if(!empty($existing_orders)) {
							$selected_vendor = $existing_orders[0]['vendor_number'];
						}
						foreach($vendors as $key => $value) { 
							$selected = ($value['vendor_number'] == $selected_vendor) ? 'selected="selected"' : ''; 
						?>
          ...
          <div class="row">
               ...
               <select name="ship_to" id="ship_to" class="form-control" required>
				<option value="">Select Center</option>
              	<?php 
					$selected_ship_to = '';
					if(!empty($existing_orders)) {
						$selected_ship_to = $existing_orders[0]['ship_to'];
					}
             ...
             <select name="bill_to" id="bill_to" class="form-control" required>
				<option value="">Select Center</option>
              	<?php 
					$selected_bill_to = '';
					if(!empty($existing_orders)) {
						$selected_bill_to = $existing_orders[0]['bill_to'];
					}
					$all_centers = $all_method->get_all_centers();
					foreach($all_centers as $key => $val){ 
			...
			<div class="form-group col-sm-4 col-xs-12">
				...
				<select name="center" id="center" class="form-control" required>
					...
					$selected_center = '';
					if(!empty($existing_orders)) {
						$selected_center = $existing_orders[0]['center'];
					}
					$centers = array(
						"CASH MEDICINE NOIDA",
						"CASH MEDICINE GGN",
