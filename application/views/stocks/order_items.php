...
<div class="col-sm-12 col-xs-12  panel panel-piluku ml-5">
  ...
  <?php foreach($data as $ky => $vl){ 
   ...
   if (!empty($vl['gstdivision']) && $vl['gstdivision'] != 0) {
    $totalgstPriceSum += (float)$vl['total_vendor_price'] / $vl['gstdivision'];
} else {
    // Optional fallback if gstdivision is 0
    $totalgstPriceSum += 0;
}
...
<div class="row" id="print_this_section" style="display:none;">
 ...
 <table class="table table-striped table-bordered table-hover" style="margin-top:30px;">
			...
			<?php foreach($data as $ky => $vl){ 
$total_vendor_price_widout_gst = (!empty($vl['gstdivision']) && $vl['gstdivision'] != 0)
    ? ($vl['total_vendor_price'] / $vl['gstdivision'])
    : 0; // or use 'N/A', '', or any fallback

			if (!empty($vl['gstdivision']) && $vl['gstdivision'] != 0) {
    $total_gst_price = $vl['total_vendor_price'] - ($vl['total_vendor_price'] / $vl['gstdivision']);
} else {
    $total_gst_price = 0; // or an appropriate fallback value
}
			?>
