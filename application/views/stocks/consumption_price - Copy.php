 <?php $all_method =&get_instance(); ?>
	...
	if(!empty($injections_arr)){
		...
		foreach ($injections_arr as $row ){
			...
			$matched = "na";
			foreach($injection_array as $ky => $row1){
				//print($row1['patient_id']);
				echo 'row: '.$row1['patient_id'].' : '.$vl['patient_id'];
				//print($vl['patient_id']);
				//echo '</br>';
				if((string) $row1['patient_id'] == (string) $vl['patient_id']){
					$matched = "dt";
					$final_price = (float) $row1['total_injections_price'] + (float) $injections_price;
					$injection_array[$ky]['total_injections_price'] = $final_price;
					break;
				}
			...
			if ($matched == "na"){
				$injection_array[] = array(
					...
					'injections_price' => $injections_price,
		...
		foreach ($CustomerMedicalData as $key => $value) {
			...
			echo '<tr class="odd gradeX">';
	        echo '<td>'.$row1['add_on'] ?? ''.'</td>';
	        echo '<td>'.$value['patient_id'].'</td>';
	        echo '<td>'."Injection".'</td>';
			echo '<td>'.$value['medical_data']['injections'].'</td>';
			echo '<td></td>';
			echo '<td>'.$value['medical_data']['injections'].'</td>';
			echo '</tr>';
		}
