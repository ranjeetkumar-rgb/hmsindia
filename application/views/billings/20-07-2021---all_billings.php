<?php $all_method =&get_instance(); //var_dump($_SERVER['REDIRECT_QUERY_STRING']);die; ?>
      ...
      <!-- Advanced Tables -->
       <!--Consultation  Tables -->
        ...
        <div class="col-sm-12 col-xs-12 patient_ledger_filter">
            ...
            <div class="form-group col-sm-4 col-xs-12">
            	<a href="<?php echo base_url().'billings/all_billings'; ?>" class="btn btn-large">Reset filter</a>
            </div>            
        </div>
            ...
            <table class="table table-striped table-bordered table-hover dataList" id="">
			  ...
			  ?>
                <tr class="odd gradeX">
                  ...
                  <td> <?php if($vl['status'] == 'approved'){?>
                      <?php if($vl['share'] == '0'){?>
                  		 <a href="javascript:void(0);" type="procedure" bill="<?php echo $vl['receipt_number']; ?>" class="disaprove_first btn btn-large">IIC share</a> <?php }else{echo $currency.$vl['center_share'];}?>
    ...
    <script>
		...
		$("a.now_disapprove").on("click", function(){
			...
			window.location.href = '<?php echo base_url();?>billings/iic_share_update/'+bill_id+'?t='+bill_type+'&s='+iic_share+'';			
		});
