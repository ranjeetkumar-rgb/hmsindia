...
<th colspan="1" style="width:5%; border:1px solid #000;"></th>
</tr>
<?php
            ...
            if (!empty($procedure_id)) {
                ...
                if ($femalemed_result) {
                    ...
                    <tr>
                        ...
                        <td colspan="1" style="font-size:14px; border:1px solid #000;">
                            <input type="checkbox" id="status_<?= $sub_procedure_counter ?>" name="status_<?= $sub_procedure_counter ?>" value="1">
                        </td>
                    </tr>
                    <?php
                }
...
if ($package_suggestion_list !== false && is_array($package_suggestion_list)) {
    foreach ($package_suggestion_list as $package_name => $group) {
        ...
        ?>
        <tr>
            <td style="font-size:14px; border:1px solid #000; vertical-align: middle; width:40%;">
...
<td width="25%" colspan="2">Rs: <input type="text" readonly="" id="name29" value="<?= number_format($grand_total_price, 2) ?>" style="border-top:0px;border-left:0px;border-right:0px;width:200px;"></td>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    while (true) {
        let priceInput = document.getElementById('price_' + counter);
        if (!priceInput) break; // Stop if no more inputs
    ...
    // Set the sum into the total_after_discount field
    document.getElementById('total_after_discount').value = total.toFixed(2);
	document.getElementById('booking_amount').value = (total * 0.10).toFixed(2);
	document.getElementById('booking_amount_40').value = (total * 0.40).toFixed(2);
	document.getElementById('booking_amount_50').value = (total * 0.50).toFixed(2);
});
</script>
