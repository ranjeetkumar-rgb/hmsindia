...
<form class="col-sm-12 col-xs-12" method="post" action="" enctype="multipart/form-data">
  ...
  <div class="row">
    <div class="col-sm-12 col-xs-12 panel panel-piluku" id="consultation_details">
         ...
         <div class="row">
          ...
          <input type="hidden" name="medicine_suggestion" value="<?php echo $billing_details['medicine_suggestion']; ?>" />
                    ...
                    <table id="medicine_list_table">
                        ...
                        <tbody>
      <?php if(!empty($billing_details['male_medicine_suggestion_list'])){  $male_medine = unserialize($billing_details['male_medicine_suggestion_list']);
      ...
      <?php if(!empty($billing_details['female_medicine_suggestion_list'])){ $female_medine = unserialize($billing_details['female_medicine_suggestion_list']);
          if(!empty($female_medine)){
					 $female_med_count = 1;
         ...
         <div class="row">
          ...
          foreach($male_investigation_suggestion_list as $key => $val){
				...
				<tr class="male_ivt_tr" id="male_invstg_<?php echo $male_ivt_count; ?>">
				    ...
				    <td><a href="javascript:void(0);" class="remove_invstg_tr" data-investg="male_invstg_<?php echo $male_ivt_count; ?>" ><i class="fa fa-minus-circle error" aria-hidden="true"></i></a></td>
      ...
      <?php if(!empty($billing_details['female_investigation_suggestion_list'])){ $female_investigation_suggestion_list = unserialize($billing_details['female_investigation_suggestion_list']);
          $female_ivt_count = 1;
				...
				<tr class="female_ivt_tr" id="fmale_invstg_<?php echo $female_ivt_count; ?>">
				
				    <td><a href="javascript:void(0);" class="remove_invstg_tr" data-investg="fmale_invstg_<?php echo $female_ivt_count; ?>" ><i class="fa fa-minus-circle error" aria-hidden="true"></i></a></td>
          ...
          <div class="row">
            ...
            <div class="row">            
              <div class="form-group col-sm-6 col-xs-12">
                  ...
                  <!-- <p id="show_disc_app" style="display:none;">Given discount is more than allowed, <a href="javascript:void(0);" accountant="<?php echo $_SESSION['logged_billing_manager']['username'];?>" id="get_discount_approval">click here 4</a> for admin approval.</p> -->
                </div>
...
<script type="text/javascript">
  ...
  $(document).on('keyup',".day_for_field",function(e) {
      ...
      $('strong#medicine_total').empty().append(grand_total.toFixed(2));
  ...
  $(document).on('change',"#billing_from",function(e) {
    ...
    $(document).on('click',".remove_medicine_tr",function(e) {
    var trid = $(this).data('medicine');
    $('tr#'+trid).remove();
