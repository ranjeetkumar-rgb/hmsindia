		...
		if($logg['status'] == true){
			...
			if(isset($_POST['action']) && isset($_POST['action']) && $_POST['action'] == 'add_item'){
				...
				if($check_center_item > 0){
					$data = $this->stock_model->update_item($_POST, $product_id, $invoice_no, $no_of_item, $brand_name, $vendor_number, $batch_number, $vendor_price, $hsn, $pack_size, $gstrate, $gstdivision, $expiry, $mrp, $date_of_purchase);
				}else{
				...
				if($data > 0){
					...
					$vndr_bill['order_qty'] = $_POST['quantity'];
				...
				if(!empty($c_counter)){
					foreach($c_counter as $key => $ccounte){
						if($_POST['consumables_name_'.$ccounte] == ''){
						    ...
						    $query = "INSERT INTO `hms_central_stock_report` (invoice_no, item_number, company, item_name, batch_number, openstock, expiry, add_date, employee_number, vendor_price, mrp, hsn, gstrate, gstdivision,enddate, quantity_out, closingstock,type, total_vendor_price_gst_excluded, total_vendor_price_gst_included, total_mrp_price, patient_id, date_of_purchase, center_number) values ('$invoice_no','$item_number','$company','$item_name','$batch_number','$open_stock','$expiry','".date("Y-m-d H:i:s")."','$employee_number','$vendor_price','$mrp','$hsn','$gstrate','$gstdivision','".date("Y-m-d")."','$quantity_out','$closingstock','Cash','$total_vendor_price_gst_excluded','$total_vendor_price_gst_included','$total_mrp_price','$patient_id','$date_of_purchase','$center_number')";
                            $result = run_form_query($query); 
	...
	public function stocks(){
		...
		if($logg['status'] == true){
			...
			if (isset($export_billing)){
				...
				$fp = fopen('php://output','w');
				$headers = 'Item Number,Company, Item Name, Batch Number, Brand Name, Vendor Number, Generic Name, Quantity, Safety Stock, Order Qty, Category, Status';
				//Add the headers
				fwrite($fp, $headers. "\r\n");
				foreach ($data as $key => $val) {//var_dump($val);die;
					$billing_from = $val['billing_from'];
					...
					$billing_at = get_center_name($val['billing_at']);
					$lead_arr = array($val['item_number'], $val['company'],$val['item_name'], $val['batch_number'],$val['brand_name'], $val['vendor_number'], $val['generic_name'], $val['quantity'], $val['safety_stock'], $val['order_qty'], $val['category'], $val['status']);
					fputcsv($fp, $lead_arr);
				}
	...
	public function stocks_reports(){
		...
		if($logg['status'] == true){
			...
			if (isset($export_billing)){
				...
				$fp = fopen('php://output','w');
				$headers = 'Item Number, Item Name, Batch Number, Opening Stock, Quantity IN, Quantity Out, Closing Stock, Vendor Price, MRP, GST RATE, Type, Date, Employee Name';
				//Add the headers
				fwrite($fp, $headers. "\r\n");
				foreach ($data as $key => $val) {//var_dump($val);die;
				   // $gst_amount = $gstdivision / $val['vendor_price'];
				    $lead_arr = array($val['item_number'], $val['item_name'], $val['batch_number'], $val['openstock'], $val['quantity_in'], $val['quantity_out'], $val['closingstock'], $val['vendor_price'],  $val['mrp'], $val['gstrate'], $val['type'], $val['add_date'], $val['employee_number']);
					fputcsv($fp, $lead_arr);
				}
				fclose($fp);
				exit();
...
public function medicine_stock(){
		...
		if($logg['status'] == true){
			...
			if (isset($export_return_medicine)){
				...
				$fp = fopen('php://output','w');
				$headers = 'Date, IIC ID, Patient Name, Hospital Id,Receipt Number, Payment Method, Medicine Name,Item Name, Quantity,Out Qty, Amount, Vendor Price, Discount, Total, Employee Number, Status, Batch Number, HSN, GST, MRP';
				//Add the headers
				fwrite($fp, $headers. "\r\n");
				foreach ($data as $key => $val) {//var_dump($val);die;
					$billing_from = $val['billing_from'];
					...
					$employee_number = get_center_name($val['employee_number']);
					$lead_arr = array($val['on_date'], $val['patient_id'], $val['patient_detail_name'],$val['hospital_id'],$val['receipt_number'], $val['payment_method'], $val['consumables_name'],$val['consumables_item_name'],$val['consumables_stock'], $val['consumables_quantity'], $val['consumables_price'], $val['consumables_vendor_price'], $val['consumables_discount_'], $val['consumables_total_'],$val['employee_number'], $val['status'], $val['consumables_batch_number'], $val['consumables_hsn'], $val['consumables_gstrate'], $val['consumables_mrp']);
					fputcsv($fp, $lead_arr);
				}
				fclose($fp);
				exit();
	...
	public function return_medicine_item(){
		...
		if($logg['status'] == true){
			...
			$data['employee_number'] = $employee_number;
			$data['data'] = $this->stock_model->get_center_item_data($item_id, $employee_number);
			//$data['categories'] = $this->stock_model->get_categories();
			$template = get_header_template($logg['role']);
			$this->load->view($template['header']);
	...
	public function transfer_stocks($ID = null) {
		...
		if ($logg['status'] == true) {
			...
			if (isset($_POST['action']) && $_POST['action'] == 'transfer_stocks') {
				...
				$data = $this->stock_model->transfer_stock_report($_POST, $item_number, $product_id, $item_name, $company, $batch_number, $openstock, $expiry, $expiry_day, $add_date, $employee_number, $vendor_price, $mrp, $hsn, $gstrate, $gstdivision, $pack_size, $brand_name, $vendor_number,$generic_name,$category, $quantity_out, $center_number, $department, $r_center_number, $r_employee_number, $r_department, $remarks);
				
				if ($data > 0) {
					// Update stock and redirect
					$update_stock = $this->stock_model->deduct_transfer_stock($item_number, $employee_number, $quantity_out, $center_number);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] == 'update_transfer_item') {
        ...
        $employee_number = $_POST['employee_number'];
        $department = $_POST['department'];
		//var_dump($this->stock_model->check_center_item_new($item_number, $center_number,  $employee_number, $department));die();
		$check_center_item_new = $this->stock_model->check_center_item_new2($item_number, $center_number,  $employee_number, $department);
		//var_dump($check_center_item_new > 0);die();
        if (!empty($check_center_item_new)) {    
		//if (!empty($check_center_item_new > 0)) {	
			// Update logic
			$data = $this->stock_model->update_transfer_item($_POST, $quantity, $center_number, $item_number, $employee_number, $department, $item_name, $company, $batch_number, $openstock, $expiry, $expiry_day, $add_date, $vendor_price, $mrp, $hsn, $gstrate, $gstdivision, $quantity_out);
			var_dump('Updated');
		} else {
			// Insert new record
			$data = $this->stock_model->transfer_stock_report_center($_POST, $item_number, $product_id, $item_name, $company, $batch_number, $quantity, $openstock, $expiry, $expiry_day, $add_date, $employee_number, $vendor_price, $mrp, $hsn, $gstrate, $gstdivision, $pack_size, $brand_name, $vendor_number, $generic_name, $category, $quantity_out, $center_number, $department);
			var_dump('Inserted'); 
			if ($data > 0) {
